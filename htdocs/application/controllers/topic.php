...
class Topic extends Front_Controller {
    ...
    public function detail($tid)
    {
        ...
        $str=$data['topic']['content'];
        $pattern="/<[img|IMG].*?src=['|\"](.*?(?:[.gif|.jpg|.png]))['|\"].*?[\/]?>/";
        preg_match_all($pattern,$str,$match);
        foreach ($match[1] as $image){
            $imgdst=substr($image,17);
            $imgsrc=substr($image,17);
            ob_start();
            ob_flush();
            flush();
          $this->topic_m->image_png_size_add($imgsrc,$imgdst);
        }
//        ob_end_flush();
...
//            echo $image;
//            echo strlen("http://localhost/uploads/image/201608/");
//            echo substr($image,52);
//        $replace1="<a href=\"$image\"><img class=\"thumbnail\"";
//            $replace2="/></a>";
//            $this->topic_m->replace("<img",$replace1,$tid);
//            $this->topic_m->replace("/>",$replace2,$tid);
//            $this->topic_m->replace($imgsrc,$imgdst,$tid);
